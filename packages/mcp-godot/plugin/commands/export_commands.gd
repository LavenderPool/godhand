...
func _export_project(params: Dictionary) -> Dictionary:
	...
	# Run the export headlessly via a separate Godot process (the running editor
	# instance cannot export itself, but it can spawn a headless exporter).
	...
	var command := '"%s" --headless --path "%s" %s "%s" "%s"' % [godot_path, project_path, flag, target_name, export_path]
	...
	if optional_bool(params, "dry_run", false):
		return success({
			...
			"command": command,
			"dry_run": true,
			"message": "dry_run=true: command not executed. Set dry_run=false to actually export.",
		})

	# Ensure output directory exists.
	var out_dir := export_path.get_base_dir()
	if not out_dir.is_empty() and not DirAccess.dir_exists_absolute(out_dir):
		DirAccess.make_dir_recursive_absolute(out_dir)

	var args := PackedStringArray(["--headless", "--path", project_path, flag, target_name, export_path])
	var output: Array = []
	var exit_code := OS.execute(godot_path, args, output, true)
	var stdout := str(output[0]) if not output.is_empty() else ""

	if exit_code != 0:
		return error(-32000, "Export failed (exit %d) for preset '%s'." % [exit_code, target_name], {
			"command": command,
			"export_path": export_path,
			"output": stdout,
			"suggestion": "Ensure export templates are installed and the preset is valid (get_export_info).",
		})
	...
	return success({
		...
		"debug": debug,
		"exit_code": exit_code,
		"exported": FileAccess.file_exists(export_path),
		"command": command,
		"output": stdout,
	})
