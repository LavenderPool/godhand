...
You have access to ~190 MCP tools that connect directly to the Godot 4 editor. You can create scenes, write scripts, simulate player input, inspect running games, export to phones, and more — all without the user leaving this conversation. Every change goes through Godot's UndoRedo system, so the user can always Ctrl+Z.

### Start here: get_godot_guide

Before building, call **`get_godot_guide`** to load curated, up-to-date workflow recipes:

```
get_godot_guide topic: "overview"        → golden rules + recommended order
get_godot_guide topic: "quickstart-2d"   → one-prompt 2D game recipe
get_godot_guide topic: "quickstart-3d"   → one-prompt 3D game recipe
get_godot_guide topic: "mobile"          → iOS/Android + touch controls
get_godot_guide topic: "optimization"    → keep it smooth on phones
```

The same content is also exposed as MCP **resources** (`godot-guide://<topic>`, `godot-skills://skills.md`) and ready-made **prompts** (`build-2d-mobile-game`, `build-3d-mobile-game`).

### High-level builders (fastest path to a playable game)

Prefer these one-call scaffolders, then refine with granular tools:

```
setup_2d_platformer_player  → CharacterBody2D + Sprite2D + CollisionShape2D + movement script + follow Camera2D
setup_3d_character          → CharacterBody3D + mesh + CollisionShape3D + SpringArm3D camera + movement script
apply_mobile_preset         → mobile renderer + stretch + texture compression + FPS cap, in one call
setup_touch_controls        → on-screen virtual joystick + action buttons wired to input actions
```
...
**Fastest path:** call `setup_2d_platformer_player` to scaffold the whole player (nodes + movement script + camera) in one step, then tweak.

**Manual path — creating a player:**
...
**2D world & polish tools:**
```
setup_camera_2d      → zoom, limits, smoothing, drag margins (attach to player to follow)
setup_parallax       → ParallaxBackground + layers for scrolling backgrounds
add_animated_sprite  → AnimatedSprite2D + SpriteFrames from a sprite sheet (hframes x vframes)
setup_light_2d       → PointLight2D / DirectionalLight2D (+ CanvasModulate for day/night)
create_tileset       → build a TileSet atlas from a texture, then tilemap_fill_rect / tilemap_set_cell
```

### 3. Build a 3D Scene

**Fastest path:** call `setup_3d_character` to scaffold a third-person player (mesh + collision + SpringArm3D camera + movement script) in one step.

```
create_scene         → root_type: Node3D
add_mesh_instance    → add primitives (box, sphere, cylinder, plane) or import .glb/.gltf
setup_csg            → CSG primitives for fast level blockouts
add_multimesh        → MultiMeshInstance3D for high-performance instancing (mobile-friendly)
setup_lighting       → add DirectionalLight3D, OmniLight3D, or SpotLight3D (preset: sun/indoor/dramatic)
...
add_reflection_probe → local reflections; add_decal → projected textures
setup_collision      → add collision shapes to physics bodies (dimension: "3d")
...
Make a game feel great on phones, not just run on them.

### Make it mobile-ready
```
apply_mobile_preset   → set renderer (mobile/gl_compatibility), stretch (canvas_items/expand),
                        VRAM texture compression (ETC2/ASTC), MSAA, and FPS cap in one call
setup_touch_controls  → virtual joystick + action buttons on a CanvasLayer, wired to input actions
apply_safe_area       → keep the HUD clear of notches / rounded corners (runtime DisplayServer safe area)
```

### Android deploy
```
list_android_devices  → confirm a device is connected via adb
deploy_to_android     → export APK + adb install -r + launch (one call)
get_android_preset_info → inspect the configured Android export preset
```

### iOS deploy (macOS + Xcode for a signed IPA)
```
get_ios_preset_info → inspect the iOS preset (bundle id, export path)
export_ios          → generate the Xcode project / IPA (returns a clear note on non-macOS hosts)
list_ios_targets    → list devices/simulators (macOS only)
```

### Mobile optimization
```
optimize_for_mobile      → prioritized recommendations (renderer, texture compression, FPS cap,
                          shadow-casting lights, CSG, draw calls); set auto_apply: true for safe fixes
get_performance_monitors → live FPS, draw calls, VRAM, object counts
analyze_scene_complexity → static complexity hotspots
add_multimesh            → collapse many repeated meshes into one draw call
```

Mobile rules of thumb: use the **Mobile renderer**, enable **ETC2/ASTC** compression, keep **few real-time shadows**, turn **glow/SSAO/SSR off**, cap FPS to **60**, and prefer **MultiMesh** for crowds/props.

## One-Prompt Game Recipes

### 2D platformer for Android (end to end)
1. `apply_mobile_preset` { renderer: "mobile", base_width: 1080, base_height: 1920 }
2. `set_input_action` for move_left/move_right/jump
...
4. `create_scene` { root_type: "Node2D", path: "res://scenes/world.tscn" }; `add_scene_instance` the player
5. `create_tileset` from a texture; `tilemap_fill_rect` the ground; `setup_collision` for solids
6. `setup_parallax` + `setup_light_2d` for atmosphere; `apply_particle_preset` for juice
7. `setup_touch_controls` { joystick: true, buttons: [{ action: "jump", text: "A" }] }
8. HUD via Control + `apply_safe_area`; `add_audio_player` for music
9. `play_scene` + `get_game_screenshot`, iterate; `optimize_for_mobile`
10. `deploy_to_android` { launch: true }

### 3D third-person for iOS (end to end)
1. `apply_mobile_preset` { renderer: "mobile" }
2. `set_input_action` for movement + jump
3. `setup_3d_character` { scene_path: "res://scenes/player.tscn", mesh_type: "CapsuleMesh" }
4. `create_scene` { root_type: "Node3D" }; `add_scene_instance` the player
5. `setup_csg` blockout (or `add_mesh_instance` + `setup_collision` dimension "3d"); `add_multimesh` for props
...
8. `play_scene` + `get_game_screenshot`, iterate; `optimize_for_mobile`
9. `export_ios` (macOS + Xcode required to finish the IPA)

## Recommended Workflow Order

When building a new game from scratch:

1. **Guide** — `get_godot_guide` (overview, then quickstart-2d / quickstart-3d / mobile)
2. **Project setup** — `get_project_info`, `set_project_setting`; for phones run `apply_mobile_preset` early
3. **Input mapping** — `set_input_action` for all player controls
4. **Player** — `setup_2d_platformer_player` or `setup_3d_character` (then refine)
5. **Main/World scene** — `create_scene`, place the player, build environment (TileMap/parallax or meshes/CSG)
6. **Game logic** — scripts for enemies, items, UI
7. **Mobile input** — `setup_touch_controls` + `apply_safe_area`
8. **Audio** — set up buses, add audio players
9. **Playtest** — `play_scene`, test with simulated input, fix bugs
10. **Polish & optimize** — animations, particles, shaders, themes; `optimize_for_mobile`
11. **Export** — `deploy_to_android` / `export_ios` / `export_project`
