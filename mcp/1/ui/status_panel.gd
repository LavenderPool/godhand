...
# Clients tab
var _connection_row: Dictionary = {}
...
func _build_clients_tab() -> void:
	...
	var lbl := Label.new()
	lbl.text = "  Project relay  —  Disconnected"
	row.add_child(lbl)

	_connection_row = {"icon": icon, "label": lbl}
...
func _process(_delta: float) -> void:
	...
	if websocket_server.has_method("is_port_stale"):
		any_stale = websocket_server.is_port_stale(DEFAULT_PORT)
...
func _update_clients_tab() -> void:
	if _connection_row.is_empty():
		return

	var icon: Label = _connection_row["icon"]
	var lbl: Label = _connection_row["label"]
	var connected := websocket_server.get_client_count() > 0
	var is_stale := false
	if websocket_server.has_method("is_port_stale"):
		is_stale = websocket_server.is_port_stale(DEFAULT_PORT)

	var relay_label := "Project relay"
	if websocket_server.has_method("get_relay_url"):
		var relay_url: String = websocket_server.get_relay_url()
		if not relay_url.is_empty():
			relay_label = relay_url

	if connected:
		var time_str := ""
		if websocket_server.has_method("get_port_connect_time"):
			var elapsed: float = websocket_server.get_port_connect_time(DEFAULT_PORT)
			if elapsed >= 0:
		...
		if websocket_server.has_method("get_port_idle_time"):
			var idle: float = websocket_server.get_port_idle_time(DEFAULT_PORT)
			if idle >= 2.0:
				idle_str = "  · idle %.0fs" % idle

		icon.text = "●"
		icon.add_theme_color_override("font_color", COLOR_CONNECTED)
		lbl.text = "  %s  —  Connected%s%s" % [relay_label, time_str, idle_str]
	elif is_stale:
		icon.text = "◐"
		icon.add_theme_color_override("font_color", COLOR_STALE)
		lbl.text = "  %s  —  ⚠ Stale (reconnecting)" % relay_label
	else:
		icon.text = "○"
		icon.add_theme_color_override("font_color", COLOR_DISCONNECTED)
		lbl.text = "  %s  —  Disconnected" % relay_label
